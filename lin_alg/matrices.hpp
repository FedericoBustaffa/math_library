...
template <typename T>
matrix<T> matrix<T>::rand(std::default_random_engine& engine, size_t rows,
                          size_t cols, T min, T max) {
    matrix<T> m(rows, cols);
...
template <typename T>
matrix<T> matrix<T>::rand(std::default_random_engine& engine, size_t dim, T min,
                          T max) {
